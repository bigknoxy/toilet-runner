...
There is no test framework. `src/test/TrackManager.test.ts` is an exported `console.log` function that nothing imports. `bun run typecheck` is the only automated gate.

Recommended: **Vitest** — reuses the existing Vite config, `jsdom` for the DOM-touching UI classes, one devDependency plus a `"test"` script, added to `ci.yml` beside `typecheck`.
